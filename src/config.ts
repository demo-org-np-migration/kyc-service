...
export const config = {
  ...
  keycloakIssuer: required('KEYCLOAK_ISSUER', 'http://keycloak.platform.svc:8080/realms/cauri-staging'),
  keycloakClientId: process.env.KEYCLOAK_CLIENT_ID ?? 'kyc-service',
  keycloakClientSecret: process.env.KEYCLOAK_CLIENT_SECRET ?? '',

  awsEndpointUrl: required('AWS_ENDPOINT_URL', 'http://localstack.platform.svc:4566'),
  ...
  veridocUrl: required('VERIDOC_URL', 'http://sandbox.veridoc-id.com'),
  veridocApiKey: process.env.VERIDOC_API_KEY ?? '',

  ledgerCoreUrl: process.env.LEDGER_CORE_URL ?? 'http://ledger-core:8080',
};
